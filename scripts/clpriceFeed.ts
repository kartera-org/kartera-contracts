...
async function main() {
    ...
    const CLPriceFeed = (await ethers.getContractFactory("CLPriceFeed")) as CLPriceFeed__factory;
    const cLPriceFeed = await CLPriceFeed.attach(clPriceFeedContract);

    let price = await cLPriceFeed.getLatestPrice(addresses[7].addr);
